...
- [Advanced module features](#advanced-module-features)
  ...
  - [Condition rule behaviour](#condition-rule-behaviour)
    - [Change input file or parameter based on wildcard](#change-input-file-or-parameter-based-on-wildcard)
    - [Change input file or parameter based on sample metadata](#change-input-file-or-parameter-based-on-sample-metadata)
  - [Adjust module based on input or intermediate file](#adjust-module-based-on-input-or-intermediate-file)
...
When you create a new module using the template (details below), you obtain the following files:
...
│   │   └── default.yaml
│   ├── envs
│   │   └── <tool>-<version>.yaml -> ../../../../envs/<tool>/<tool>-<version>.yaml
│   ├── etc
│   └── schemas
...
This section simply lists the individuals who have contributed to the module in one way or another. The `Original Author` refers to the person who wrote the Snakefile or script that was used to create the module. The `Module Author` refers to the person who either adapted a previously written Snakefile/script or created the module from scratch. Finally, the `Contributors` refers to the list of individuals who have contributed to the module over time, mainly through incremental version updates.
...
There are a few standard components for the module setup and some optional components. Importing standard modules such as `os` (for the `os.remove()` function) is optional. On the other hand, importing the `oncopipe` module is required because it offers a suite of functions that greatly simplify the process of developing modules and enhance user-facing customization. For brevity, the module is commonly imported with `import oncopipe as op`, which allows the functions to be accessible using the `op` prefix/namespace (_e.g._ `op.as_one_line()`).
...
#### Change input file or parameter based on wildcard

More details will be added later.

#### Change input file or parameter based on sample metadata

More details will be added later.

### Adjust module based on input or intermediate file
