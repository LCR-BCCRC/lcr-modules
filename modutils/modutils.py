...
import os
from os.path import isdir, join, relpath, split, normpath
...
def load_samples(file_path, lowercase_cols=LOWERCASE_COLS, mapper=None, **maps):
    ...
    for col in lowercase_cols:
        if col not in samples.columns:
            logger.warn(
                f"The `{col}` column does not exist " "in the samples data frame."
            )
            continue
...
def generate_runs_for_patient(
    ...
    if run_unpaired_tumours_with == "unmatched_normal" and unmatched_normal is None:
        msg = (
...
def generate_runs_for_patient_and_seqtype(samples, pairing_config, **kwargs):
    ...
    if len(seq_type_set) != 1:
        msg = (
            "This function is only meant to be run on groups of samples "
    ...
    if pconfig["run_unpaired_tumours_with"] == "unmatched_normal":
        ...
        runs = generate_runs_for_patient(
            samples, **pconfig, **kwargs, unmatched_normal=unmatched_normal
        )
    else:
...
def walk_through_dict(
    dictionary, end_fn=print, end_depth=None, trace=None, result=None, **kwargs
):
    ...
    for k, v in dictionary.items():
        ...
        if isinstance(v, dict) and len(current_trace) < end_depth:
            result[k] = {}
            walk_through_dict(v, end_fn, end_depth, current_trace, result[k], **kwargs)
        else:
...
def generate_runs(samples, sample_class, subgroups=[], **kwargs):
    ...
    runs = walk_through_dict(
        patients, generate_runs_for_patient_and_seqtype, end_depth, **kwargs
    )
    while end_depth > 0:
...
def setup_module(config, name, version, subdirs, **kwargs):
    ...
    subfields = ["inputs", "dirs", "conda_envs", "options", "threads", "memory"]
    for subfield in subfields:
        if subfield not in mconfig:
            logger.warning(
                f"Subfield '{subfield}' is missing from config " f"for module {name}"
            )
    ...
    runs = generate_runs(
        ...
        unmatched_normals=unmatched_normals,
        **kwargs,
    )
...
def setup_subdirs(module_config, subdirs):
    if "_parent" in subdirs:
        raise ValueError(
            "You cannot have a sub-directory called '_parent'. "
