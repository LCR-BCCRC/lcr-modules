...
CFG = op.setup_module(
    ...
    subdirectories = ["inputs", "cnvkit_targets_bed", "cnvkit_autobin", "cnvkit_coverage", "cnvkit_pon_cnn", "cnvkit_flat_ref_cnn",
                        "purecn_intervals", "purecn_mutect2", "pon_for_mutect2", "purecn_merge_vcfs",
...
localrules:
    ...
    _panel_of_normals_purecn_sort_intervals,
...
FLAT_REF_COMBOS = zip(CFG["options"]["cnvkit"]["flat_ref_combos"]["genome_builds"], CFG["options"]["cnvkit"]["flat_ref_combos"]["capture_spaces"])
META = CFG["samples"][[x not in FLAT_REF_COMBOS for x in zip(CFG["samples"]['genome_build'], CFG["samples"]['capture_space'])]]
...
rule _panel_of_normals_cnvkit_autobin:
    input:
        access = str(rules._panel_of_normals_cnvkit_accessible_main_chrs.output.access_main),
        bam = _get_normal_bams_per_combo,
...
rule _panel_of_normals_cnvkit_flat_ref_antitargets:
    input:
        ...
        access_main = str(rules._panel_of_normals_cnvkit_accessible_main_chrs.output.access_main)
    output:
...
rule _panel_of_normals_purecn_get_mappability:
    ...
    params:
        ...
        }[config["lcr-modules"]["panel_of_normals"]["options"]["genome_builds_map"].get(w.genome_build, w.genome_build)]
    conda:
        CFG["conda_envs"]["wget"]
    shell:
        op.as_one_line("""
...
rule _panel_of_normals_purecn_setinterval:
    ...
    params:
        genome = lambda w: config["lcr-modules"]["panel_of_normals"]["options"]["genome_builds_map"][w.genome_build],
    ...
    shell:
        op.as_one_line("""
            ...
            --fasta {input.fasta} --out-file {output.intervals}
...
# Downstream GATK commands need the interval files to be in the same chr order as the fasta/dict
rule _panel_of_normals_purecn_sort_intervals:
    input:
    ...
    shell:
        op.as_one_line("""
        awk -F "[:-]" -v OFS="\t" '{{print $1,$2,$3}}' {input.gatk_targets} | bedtools sort -faidx {input.fai} -i - | awk -F"\t" '{{print $1":"$2"-"$3}}'  > {output.targets_sorted} 2> {log}
        """)
...
rule _panel_of_normals_purecn_mutect2_germline:
    input:
        ...
        target_regions = str(rules._panel_of_normals_purecn_sort_intervals.output.targets_sorted)
    output:
...
def _get_mutect2_chr_vcfs(wildcards):
    ...
    with open(checkpoints._panel_of_normals_input_chroms_withY.get(**wildcards).output.txt) as f:
        chrs = f.read().rstrip("\n").split("\n")
    vcfs = expand(
        str(rules._panel_of_normals_purecn_mutect2_germline.output.vcf),
        chrom = chrs,
        allow_missing = True
    )
    return(vcfs)
...
def _get_mutect2_chr_tbis(wildcards):
    ...
    with open(checkpoints._panel_of_normals_input_chroms_withY.get(**wildcards).output.txt) as f:
        chrs = f.read().rstrip("\n").split("\n")
    tbis = expand(
        str(rules._panel_of_normals_purecn_mutect2_germline.output.tbi),
        chrom = chrs,
        allow_missing = True
    )
    return(tbis)
...
def _get_mutect2_chr_stats(wildcards):
    ...
    with open(checkpoints._panel_of_normals_input_chroms_withY.get(**wildcards).output.txt) as f:
        chrs = f.read().rstrip("\n").split("\n")
    stats = expand(
        str(rules._panel_of_normals_purecn_mutect2_germline.output.stats),
        chrom = chrs,
        allow_missing = True
    )
    return(stats)
...
def _get_mutect2_chr_f1r2(wildcards):
    ...
    with open(checkpoints._panel_of_normals_input_chroms_withY.get(**wildcards).output.txt) as f:
        chrs = f.read().rstrip("\n").split("\n")
    f1r2 = expand(
        str(rules._panel_of_normals_purecn_mutect2_germline.output.f1r2),
        chrom = chrs,
        allow_missing = True
    )
    return(f1r2)
...
# This step cannot be combined with others below, because the VCFs needed to be
# formatted differently for the GATK steps that make the MuTect2 PON VCF vs
# making the merged VCF that is used in making the PureCN NormalDB
...
rule _panel_of_normals_purecn_format_gatk_vcf:
    ...
    output:
        vcf = CFG["dirs"]["pon_for_mutect2"] + "{seq_type}--{genome_build}/{capture_space}/{sample_id}/{sample_id}_for_mutect2_pon.vcf.gz",
        tbi = CFG["dirs"]["pon_for_mutect2"] + "{seq_type}--{genome_build}/{capture_space}/{sample_id}/{sample_id}_for_mutect2_pon.vcf.gz.tbi"
    log:
        CFG["logs"]["pon_for_mutect2"] + "{seq_type}--{genome_build}/{capture_space}/{sample_id}_format_gatk_vcf.log"
    conda:
        CFG["conda_envs"]["bcftools"]
    resources:
        **CFG["resources"]["purecn"]["post_vcf"]
...
def _get_gatk_vcfs_per_combo(wildcards):
    ...
    samples = tbl[(tbl.genome_build == wildcards.genome_build) & (tbl.capture_space == wildcards.capture_space)]["sample_id"].tolist()
    normals = expand(
        str(rules._panel_of_normals_purecn_format_gatk_vcf.output.vcf),
        sample_id = samples,
        allow_missing = True)
    return normals

def _get_gatk_tbis_per_combo(wildcards):
    tbl = META
    samples = tbl[(tbl.genome_build == wildcards.genome_build) & (tbl.capture_space == wildcards.capture_space)]["sample_id"].tolist()
    normals = expand(
        str(rules._panel_of_normals_purecn_format_gatk_vcf.output.tbi),
        sample_id = samples,
        allow_missing = True)
    return normals
...
rule _panel_of_normals_purecn_samples_map:
    input:
        normals = _get_gatk_vcfs_per_combo
    output:
        samples_map = CFG["dirs"]["pon_for_mutect2"] + "{seq_type}--{genome_build}/{capture_space}/samples_map.txt",
        done = CFG["dirs"]["pon_for_mutect2"] + "{seq_type}--{genome_build}/{capture_space}/samples_map.done"
    shell:
        """
...
rule _panel_of_normals_purecn_gatk_genomicsDbimport:
    input:
        normal = _get_gatk_vcfs_per_combo,
        normal_tbi = _get_gatk_tbis_per_combo,
        samples_map = str(rules._panel_of_normals_purecn_samples_map.output.samples_map),
...
rule _panel_of_normals_purecn_merge_vcfs:
    ...
    output:
        normal_panel = CFG["dirs"]["purecn_merge_vcfs"] + "{seq_type}--{genome_build}/{capture_space}/{capture_space}_purecn_pon.vcf.gz",
        normal_panel_tbi = CFG["dirs"]["purecn_merge_vcfs"] + "{seq_type}--{genome_build}/{capture_space}/{capture_space}_purecn_pon.vcf.gz.tbi"
    log:
        CFG["logs"]["purecn_merge_vcfs"] + "{seq_type}--{genome_build}/{capture_space}/purecn_pon.vcf.log"
    conda:
        CFG["conda_envs"]["bcftools"]
    threads: 1
    shell:
...
def _get_gatk_chr_cov_depth(wildcards):
    ...
    with open(checkpoints._panel_of_normals_input_chroms_withY.get(**wildcards).output.txt) as f:
        chrs = f.read().rstrip("\n").split("\n")
    coverage = expand(
        str(rules._panel_of_normals_purecn_gatk_depthOfCoverage.output.coverage),
        chrom = chrs,
        allow_missing = True
    )
    return(coverage)
...
def _get_gatk_chr_cov_stats(wildcards):
    ...
    with open(checkpoints._panel_of_normals_input_chroms_withY.get(**wildcards).output.txt) as f:
        chrs = f.read().rstrip("\n").split("\n")
    statistics = expand(
        str(rules._panel_of_normals_purecn_gatk_depthOfCoverage.output.statistics),
        chrom = chrs,
        allow_missing = True
    )
    return(statistics)
...
rule _panel_of_normals_purecn_coverage_list:
    ...
    output:
        cov_list = CFG["dirs"]["purecn_coverage"] + "{seq_type}--{genome_build}/{capture_space}/cov_list.txt"
    shell:
        op.as_one_line("""
...
rule _panel_of_normals_purecn_database_cnvkit:
    ...
    output:
        mapping_bias = CFG["dirs"]["purecn_NormalDB"] + "{seq_type}--{genome_build}/{capture_space}/purecn_cnvkit_normal/mapping_bias_{capture_space}_{genome_build}.rds"
    log:
...
rule _panel_of_normals_purecn_database_denovo:
    ...
    output:
        mapping_bias = CFG["dirs"]["purecn_NormalDB"] + "{seq_type}--{genome_build}/{capture_space}/purecn_denovo_normal/mapping_bias_{capture_space}_{genome_build}.rds",
        normal_db = CFG["dirs"]["purecn_NormalDB"] + "{seq_type}--{genome_build}/{capture_space}/purecn_denovo_normal/normalDB_{capture_space}_{genome_build}.rds"
    log:
...
rule _panel_of_normals_purecn_output_database_denovo:
    input:
        ...
        normal_db = str(rules._panel_of_normals_purecn_database_denovo.output.normal_db)
    output:
