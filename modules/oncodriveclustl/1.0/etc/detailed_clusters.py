...
def reformat_clusters(clusters):
    ...
    merged_clusters['COORDINATES'] = merged_clusters['COORDINATES'].apply(lambda x: [int(i) for i in x.split(',')])
    merged_clusters['COORDINATES'] = merged_clusters['COORDINATES'].apply(lambda x: list(range(x[0], x[1]+1)) if len(x) > 1 else [x[0]])
...
def main(args):
    ...
    # Write output file
    clusters_expanded.to_csv(args['output'], sep="\t",na_rep="NA", index=False)
