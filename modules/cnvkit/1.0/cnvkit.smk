...
CFG = op.setup_module(
    ...
    subdirectories = ["inputs", "cnvkit", "outputs"],
)
...
rule _cnvkit_step_1:
    ...
    output:
        seg = CFG["dirs"]["cnvkit"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/output.seg"
    log:
...
rule _cnvkit_step_2:
    input:
        seg = str(rules._cnvkit_step_1.output.seg)
    output:
        seg = CFG["dirs"]["cnvkit"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/output.filt.seg"
    log:
...
rule _cnvkit_output_seg:
    input:
        seg = str(rules._cnvkit_step_2.output.seg)
    output:
...
rule _cnvkit_all:
    input:
        expand(
            [
                str(rules._cnvkit_output_seg.output.seg),
