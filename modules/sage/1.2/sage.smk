...
# Download SAGE-specific reference files
...
rule _run_sage:
    ...
    shell:
        ...
        -cp {params.sage_jar} com.hartwig.hmftools.sage.SageApplication
        -threads {threads}
        {params.opts}
...
rule _sage_filter_vcf:
    ...
    output:
        vcf_passed = CFG["dirs"]["vcf"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/passed.vcf.gz",
        vcf_passed_tbi = CFG["dirs"]["vcf"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/passed.vcf.gz.tbi"
    log:
...
rule _sage_output_vcf:
    input:
        ...
        combined_tbi = str(rules._sage_filter_vcf.output.vcf_passed_tbi)
    output:
    ...
    run:
        ...
        op.relative_symlink(input.combined_tbi, output.combined_tbi, in_module=True)
...
rule _sage_all:
    input:
        expand(
            ...
            str(rules._sage_output_vcf.output.combined_tbi),
            ],
            zip,
