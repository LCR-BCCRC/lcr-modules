...
class SomaticSNVCaller(object):
    ...
    def __init__(self, bam_n, bam_t, ref, outprefix,
                 bed=None, dbsnp=None, continue_interrupted=False, normal_only=False):
        """init function
        """
        if normal_only:
            assert all([bam_n, ref, outprefix]), (
                "Missing mandatory arguments")
        elif continue_interrupted:
            assert all([bam_t, ref, outprefix]), (
                "Missing mandatory arguments")
        else:
        ...
        if not self.continue_interrupted:
            for f in self.outfiles:
                assert not os.path.exists(f), (
                    "Line 185: Cowardly refusing to overwrite already existing file %s" % f)
    ...
    def call_rlx(self, sample_type):
        ...
        print(f"relaxed calling on {sample_type}")
        ...
        if self.continue_interrupted:
            if os.path.exists(out_vcf):
                print(f"path exists: {out_vcf}")
                assert os.path.exists(out_log), (
                    "%s exists but %s is missing." % (out_vcf, out_log))
                LOG.info("Skipping rlx call on %s" % sample_type)
                print("Skipping rlx call on %s" % sample_type)
                #LOG.info("Parsing number of tests from log file %s" % out_log)
                #print("Parsing number of tests from log file %s" % out_log)
                #fh = open(out_log, 'r')
                #print(f"opening {out_log}")
                #elines = [l.decode().replace("stderr: ", "") for l in fh.readlines()]
                #print(f"closing {out_log}")
                #fh.close()
                #print(f"Parsing number of tests from log file {out_log} results in {elines}")
                return(1,1)
    ...
    def run(self):
        ...
        if not self.continue_interrupted:
            ...
            print(f"{num_subst_tests} and {num_indel_tests}")
...
def cmdline_parser():
    ...
    basic.add_argument("-t", "--tumor",
                        help="Tumor BAM file")
    basic.add_argument("-no","--normal_only", action="store_true",
                        help="only process the normal sample")
