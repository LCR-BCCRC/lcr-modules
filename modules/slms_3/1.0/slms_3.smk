...
# bcftools annotate is very slow if one of the VCF files is quite large (see https://github.com/samtools/bcftools/issues/1199#issuecomment-624713745)
# to get around this, lets tell bcftools to only look at regions containing the variants
# i.e. lets create a BED file of all variants, and "restrict" annotations to those regions
rule _slms_3_generate_sites:
    input:
        vcf = CFG_SLMS3["dirs"]["inputs"] + "vcf/{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}.{tool}.vcf.gz",
        fai = reference_files("genomes/{genome_build}/genome_fasta/genome.fa.fai")
    output:
        bed = temp(CFG_SLMS3["dirs"]["inputs"] + "vcf/{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}.{tool}.sites.bed")
    params:
        slop_dist = 20
    conda:
        CFG_SLMS3["conda_envs"]["bedtools"]
    shell:
        op.as_one_line("""
        zcat {input.vcf} | grep -v ^# |
        awk -F "\\t" \'BEGIN{{OFS=FS}};{{print($1, $2, $2)}}\' |
        bedtools slop -g {input.fai} -b {params.slop_dist} |
...
rule _slms_3_annotate_strelka_gnomad:
    input:
        ...
        # The rule which generates the sites file is general for all tools, so we need to let snakemake unpack the wildcards itself
        sites =  CFG_SLMS3["dirs"]["inputs"] + "vcf/{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}.strelka.combined.sites.bed",
    ...
    shell:
        op.as_one_line("""
        bcftools annotate --threads {threads} --regions-file {input.sites}
...
rule _slms_3_annotate_lofreq_gnomad:
    input:
        ...
        sites =  CFG_SLMS3["dirs"]["inputs"] + "vcf/{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}.lofreq.snvs.sites.bed",
        gnomad = reference_files("genomes/{genome_build}/variation/af-only-gnomad.{genome_build}.vcf.gz")
    output:
...
rule _slms_3_annotate_sage_gnomad:
    input:
        ...
        sites =  CFG_SLMS3["dirs"]["inputs"] + "vcf/{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}.sage.combined.sites.bed",
        gnomad = reference_files("genomes/{genome_build}/variation/af-only-gnomad.{genome_build}.vcf.gz")
    output:
