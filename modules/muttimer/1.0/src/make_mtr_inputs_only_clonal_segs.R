...
makeCNV =function(subclones_file=subclones_file , cellularity_file = cellularity_file, outfile = out_cnv){
 ...
 cnv2 <- cnv1[is.na(cnv1$nMaj2_A),]
 cnv2$width<-cnv2$endpos-cnv2$startpos
 cnv2$strand<-rep("*",nrow(cnv2))
 cnv2$clonal_frequency = rep(cellularity$cellularity)

 cnv3<-cnv2[,c("chr","startpos","endpos","width","strand", "nMaj1_A","nMin1_A","clonal_frequency")]
 colnames(cnv3)<-c("seqnames","start","end","width","strand", "major_cn","minor_cn", "clonal_frequency")
 write.table(cnv3, file =outfile , row.names = FALSE, sep = "\t", quote = FALSE)
