...
rule _lymphgen_add_sv:
    ...
    output:
        sample_annotation = CFG["dirs"]["lymphgen_input"] + "{outprefix}_sample_annotation.{cnvs_wc}.{sv_wc}.tsv"
    params:
...
# Use this as the target rule if you don't have access to LymphGen source code
# Files generated by this rule can be uploaded to the LymphGen webtool. 
rule _lymphgen_generate_input: 
    input: 
        expand(
            [
                CFG["dirs"]["lymphgen_input"] + "{outprefix}_sample_annotation.{cnvs_wc}.{sv_wc}.tsv",
                CFG["dirs"]["lymphgen_input"] + "{outprefix}.{cnvs_wc}_mutation_flat.tsv"
            ], 
            outprefix = outprefix,
            cnvs_wc = cnvs_wc,
            sv_wc = sv_wc,
