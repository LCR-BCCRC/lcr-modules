...
def _mutect2_get_capture_intervals(wildcards):
    # For capture samples, obtain the appropriate capture space interval list
    # First, obtain the entry in the sample table corresponding to this sample
    # If this sample was generated via capture sequencing, return the interval list
    # to the specified 'tumor_capture_space'. Otherwise, return an empty string
    # The interval lists are generated via the reference workflow

    # Get the sample table entry corresponding to this sample
    sample_table = CFG["runs"]
    ...
    if len(sample) != 1:
        raise AssertionError("Found %s matches when examining the sample table for pair \'%s\' \'%s\' \'%s\' \'%s\'" % (wildcards.tumour_id, wildcards.normal))
    ...
    if sample.iloc[0]['tumour_seq_type'] == "capture":
       ...
       # Sanity check
        ...
        return reference_files("genomes/" + wildcards.genome_build + "/capture_space/" + capture_space + ".interval_list")
    else:
        # For uncaptured samples, no capture space needed
        return ""
