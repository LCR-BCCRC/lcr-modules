"""
Variants are filtered for unmatched samples (no matched normal).

1) Allele frequency (AF) is recalculated based on t_alt_count / t_depth.
2) gnomAD filtering: keep if all gnomAD AF population columns < threshold OR hotspot OR is_phased.
3) Blacklist and hotspot positions annotated (both optional).
...
5) Blacklisted variants are removed (blacklist always overrides exemptions).
6) VAF filtering: only non-hotspot, non-phased variants are removed if AF < min_tumour_vaf.
   All variants meeting the AF threshold (including hotspot/phased) get the "high_vaf" tag.
7) Read support filtering (tumor alt count, tumor depth, UMI thresholds).
   hotspot OR phased variants bypass these filters entirely.
8) variant_source accumulates all applicable reasons: hotspot, phase_group, high_vaf.
9) Final filtered MAF written.
...
def get_args():
    """Define and parse CLI arguments for the unmatched variant filtering pipeline."""
    p = argparse.ArgumentParser(
        description="Filter variants for unmatched samples with hotspot/phasing exemptions."
    )
    # I/O
    ...
    # UMI thresholds
    p.add_argument('--min_UMI_max', type=int, default=3, help='Minimum UMI_max count (if column exists).')
    # background rate filtering
...
class PipelineConfig:
    """Configuration for the unmatched variant filtering pipeline."""
    input_maf: str
    output_maf: str
    blacklist: str = ""
    hotspots: str = ""
    gnomad_threshold: float = 0.001
    min_alt_depth_tum: int = 3
    min_t_depth: int = 200
...
class UnmatchedVariantFilterPipeline:
    """Variant filtering pipeline for unmatched samples with hotspot/phasing exemptions."""
    ...
    def filter_variants(self) -> pd.DataFrame:
        ...
        df = self.mark_phased_vars(df)  # adds is_phased, phase_set_size, seeds variant_source
        ...
        df = self.filter_by_read_support(
            ...
            min_UMI_max=self.cfg.min_UMI_max,
        )
    ...
    def filter_by_read_support(
        ...
        """
        Apply depth/alt/UMI filters to non-exempt variants only.
        Exempt variants (hotspot OR is_phased) always pass.
        """
        required = {"t_depth", "t_alt_count", "AF", "hotspot", "is_phased"}
        ...
        # UMI filter (if column exists)
        if "UMI_max" in out.columns:
            umi_mask = out["UMI_max"] >= min_UMI_max
            base_mask &= umi_mask
...
def main():
    ...
    pipeline = UnmatchedVariantFilterPipeline(cfg)
    df = pipeline.filter_variants()
    print(f"Finished unmatched filtering, {df.shape[0]} variants remain")
