...
CFG = op.setup_module(
    ...
    subdirectories = ["inputs", "clairs_to", "outputs"],
)
...
rule _clairs_to_step_1:
    input:
        tumour_bam = CFG["dirs"]["inputs"] + "bam/{seq_type}--{genome_build}/{tumour_id}.bam",
        normal_bam = CFG["dirs"]["inputs"] + "bam/{seq_type}--{genome_build}/{normal_id}.bam",
        fasta = reference_files("genomes/{genome_build}/genome_fasta/genome.fa")
    output:
        vcf = CFG["dirs"]["clairs_to"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/output.vcf"
    log:
        stdout = CFG["logs"]["clairs_to"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/step_1.stdout.log",
        stderr = CFG["logs"]["clairs_to"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/step_1.stderr.log"
    params:
...
# TODO: Replace example rule below with actual rule
rule _clairs_to_step_2:
    input:
        vcf = str(rules._clairs_to_step_1.output.vcf)
    output:
        vcf = CFG["dirs"]["clairs_to"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/output.filt.vcf"
    log:
        stderr = CFG["logs"]["clairs_to"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/step_2.stderr.log"
    params:
...
rule _clairs_to_output_vcf:
    input:
        vcf = str(rules._clairs_to_step_2.output.vcf)
    output:
