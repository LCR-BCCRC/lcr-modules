...
rule _run_battenberg:
    ...
    shell:
       op.as_one_line("""
        if [[ $(head -c 4 {params.fasta}) == ">chr" ]]; then chr_prefixed_flag='--chr_prefixed_genome'; else chr_prefixed_flag=''; fi;
        ...
        -o {params.out_dir} $chr_prefixed_flag --sex $sex --cpu {threads} >> {log.stdout} 2>> {log.stderr} &&
...
rule _battenberg_fill_subclones:
    ...
    log:
        stderr = CFG["logs"]["fill_regions"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}/{tumour_id}_fill_subclones.stderr.log"
    threads: 1
    group: "battenberg_post_process"
    params:
