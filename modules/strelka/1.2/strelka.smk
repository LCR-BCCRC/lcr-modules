...
def _strelka_get_bedz(wildcards):
    # Obtain the appropriate capture region BED file based on the sequencing type and capture space
    # First, obtain the entry in the sample table corresponding to this sample
    # If this sample was generated via capture sequencing, return the BED file corresponding
    # to the specified 'tumor_capture_space'. Otherwise, return a BED corresponding to all the
    # chromosome primary contigs
    # These BED(.gz) files are generated via the reference workflow

    # Get the sample table entry corresponding to this sample
    sample_table = config['lcr-modules']['strelka']['runs']  # No idea why I have to do this with config here, but CFG in the manta workflow
    sample = sample_table.loc[(sample_table['tumour_sample_id'] == wildcards.tumour_id) &
                (sample_table['normal_sample_id'] == wildcards.normal_id) & (sample_table['tumour_genome_build'] == wildcards.genome_build) &
                (sample_table['tumour_seq_type'] == wildcards.seq_type)]
    if len(sample) != 1:
        raise AssertionError("Found %s matches when examining the sample table for pair \'%s\' \'%s\' \'%s\' \'%s\'" % (wildcards.tumour_id, wildcards.normal_id, wildcards.genome_build, wildcards.seq_type))

    # Is this sample generated using a capture-based approach? If so, grab the capture space
    if sample.iloc[0]['tumour_seq_type'] == "capture":
        capture_space = sample.iloc[0]['tumour_capture_space']
        # Sanity check
        if capture_space == "NA":
            raise AttributeError("Sample \'%s\' is specified as a capture, but no capture space was provided" % wildcards.tumour_id)
        return reference_files("genomes/" + wildcards.genome_build + "/capture_space/" + capture_space + ".bed.gz")
    else:
        # For uncaptured samples, just use a BED file for all the contigs
