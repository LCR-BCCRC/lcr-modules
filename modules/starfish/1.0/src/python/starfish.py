...
def index_vcf(vcf_fname, overwrite=True):
    if overwrite:
        call(['/home/ddodani/bin/htslib-1.9/tabix', '-f', vcf_fname])
    else:
        call(['/home/ddodani/bin/htslib-1.9/tabix', vcf_fname])
...
def concat(vcfs, out, remove_duplicates=True, debug=False):
    assert len(vcfs) > 1
    cmd = ['/home/ddodani/bin/bcftools/bin/bcftools', 'concat', '-a', '-Oz', '-o', out]
...
def merge(vcfs, out, force_samples=True, debug=False):
    assert len(vcfs) > 1
    cmd = ['/home/ddodani/bin/bcftools/bin/bcftools', 'merge', '-Oz', '-o', out]
    if force_samples:
